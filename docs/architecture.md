...
main.rs                      entry point — configures the layer-shell window and starts the iced runtime
 ...
 └─ app.rs                   top-level state machine (Trebuchet struct)
     │                         owns Config, app list, and all component instances
...
`app.rs` is a pure message router — it holds all component structs but only delegates to the active one. Cross-component navigation (e.g. typing `/cmd` in the app launcher to switch to command mode) is communicated via `ComponentEvent::CommandInvoked(SlashCommand, args)` returned from the active component and dispatched centrally in `apply_event()`.
...
Slash commands (`/mv`, `/cmd`, `/config`, `/app`) are detected in two ways:
...
**Rationale:** Space-triggered dispatch makes mode switching feel instantaneous — `/cmd ` drops the user straight into command mode as soon as the space is typed, with the `/cmd ` prefix consumed automatically. The Enter path is a fallback for users who type the command and then press Enter.
...
| `serde` / `serde_json` | 1 | JSON parsing (hyprctl output, icon manifest) |
...
- **`xdg-open`** — optional, for opening links
