use iced::{
    alignment,
    widget::{column, container, markdown, pick_list, row, scrollable, text, Space},
    Alignment, Background, Border, Color, Element, Font, Length, Padding,
};

use super::panel::{icon_btn, COPY_ICON};
...
fn md_settings(theme: &Theme) -> markdown::Settings {
    let (code_bg, code_text, link) = (theme.ai_code_background, theme.ai_code_text, theme.ai_link);
...
pub fn ai_panel<'a, Msg: Clone + 'a>(
    ...
    let (idle_color, error_color, text_color) = (theme.ai_idle, theme.ai_error, theme.search_text);
    let body: Element<'a, Msg> = match status {
        ...
        AiStatus::Error(err) => {
            container(text(format!("Error: {err}")).size(15).color(error_color))
                .width(Length::Fill)
                .height(Length::Fill)
                .into()
        }
